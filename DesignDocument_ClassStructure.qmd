...
<!-- define new callout types? Seems hard: https://github.com/quarto-dev/quarto-cli/discussions/7753 -->
...
::: {.callout-note}
**BMB**: Do we need "Structured" in all of these sub-class names? i.e. we could call them `CorrelationCov`, `TimeSeriesCov`, `SpatialCov`, `PhyloCov` ...
:::
...
Also, there are a couple of subtleties about parameterization, don't know whether they needed to be noted/taken into account here or not. 

* `lme4` uses a *non-logged* Cholesky factor parameterization (technically, the parameterization is $\theta = \mathrm{vech}(\mathrm{Chol}(\Sigma))$ where $\mathrm{vech}$ is the [half-vectorization operator](https://en.wikipedia.org/wiki/Vectorization_(mathematics)) ...
* `glmmTMB` uses a different parameterization (see its [covariance structure vignette](https://cran.r-project.org/web/packages/glmmTMB/vignettes/covstruct.html)), which separates the correlation matrix from the standard deviation vector. I think it would be possible (although not necessarily worth it) to allow this parameterization, by putting this translation machinery into the `calcLambda` hook function (or whatever we'll calling it ...) (again, cf `pdLogChol` vs `pdNatural` in `nlme` ...
* there are also a variety of different ways to parameterize the reduced-rank/factor-analytic covariance structure

We may need to consider a hook/method for setting starting values for a given covariance structure? (`rr` is especially sensitive/tricky)
:::
...
::: {.callout-note}
**BMB**: we may need separate functions for (1) initially building the *structure* of Lambda (i.e., what is the sparsity pattern?) and (2) computing a new set of values and poking them into the appropriate slots in an existing Lambda matrix. Note that `lme4` uses `Lambdat` internally throughout, to improve computational efficiency (see the JSS paper for details)

We don't necessarily need separate machinery for classes with different numbers of parameters: you can pass a list of parameters and apply `do.call()` ...

Finally, we do need to think about how/where to handle any additional parameters. Current `lme4` only has three parameter vectors: `beta` for fixed effects, `theta` for parameters controlling random effects, `b` for latent variables. Do we need additional parameter vectors to handle the complexity described here, or does everything get stuffed into `theta` ?
:::
